...
class Test:
    ...
    def combobox(self):
        ...
        self.points = ttk.Combobox(master=self.settings_frame, width=10, height=4, values=Test.points,
                                   state='readonly')
    ...
    def createPoint(self):
        ...
        Test.points.append([int(self.point_x.get()), int(self.point_y.get())])
        for i in Test.points:
            if type(i) is not str:
                Test.points.remove(i)
                Test.points.append(str(i))
        self.points['values'] = Test.points
